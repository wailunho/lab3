...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
		...
		// f_pos-2 to account for the "." and ".."
...
allocate_block(void)
{
	...
	{
		if(bitvector_test(bitmap_start, i) == 1)
		{
			//free block is found, allocate block and mark it non-free
...
{
	/* EXERCISE: Your code here */
	uint32_t bounds = ospfs_super->os_firstinob;
	
	bounds += ospfs_super->os_firstinob + ospfs_super->os_ninodes/OSPFS_BLKINODES;
...
indir_index(uint32_t b)
{
	...
	if( b >= (OSPFS_NDIRECT + OSPFS_NINDIRECT))
	{
		uint32_t blockoffset = b - ( OSPFS_NDIRECT + OSPFS_NINDIRECT);
		return blockoffset/ OSPFS_NINDIRECT;
	}
...
add_block(ospfs_inode_t *oi)
{
	...
	// store current number of blocks in file
  	...
  	if(newblock_no == 0)
	{
  	...
  	new_block = ospfs_block(newblock_no);
...
remove_block(ospfs_inode_t *oi)
{
	...
	uint32_t x = ospfs_size2nblocks(oi->oi_size);
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	if( (*f_pos + count) > oi->oi_size)
	{
		result = change_size(oi, (*f_pos + count) );
	}
	...
	while (amount < count && retval >= 0) {
		...
		result = copy_from_user(data, buffer, n);
		//failed
		if(result != 0)
			return -EFAULT;
...
ospfs_link(struct dentry *src_dentry, struct inode *dir, struct dentry *dst_dentry) {
	...
	d = create_blank_direntry(dir_oi);
	if(IS_ERR(d))
		return PTR_ERR(d);

	d->od_ino = src_dentry->d_inode->i_ino;
	memcpy(d->od_name, dst_dentry->d_name.name, dst_dentry->d_name.len);
...
ospfs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
	...
	while(entry_ino < ospfs_super->os_ninodes)
	{
		...
		if(new_inode != NULL && new_inode->oi_nlink == 0)
		{
			new_inode->oi_nlink++;
			break;
		}
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
	...
	/* Execute this code after your function has successfully created the
	 ...
	 // Find an open inode
  	...
  	strncpy(d->od_name, dentry->d_name.name, dentry->d_name.len);
  	d->od_name[dentry->d_name.len] = 0;
  	d->od_ino = entry_ino;
